...
import { ChartDataConfig, ChartStyleConfig } from '@/types/ChartConfig';
import { getColumnRenderName, getUnusedHeaderRows } from '@/utils/chartHelper';
  ...
  memo(
    ({
      ...
      const makeSameLinageRows = (rowAncestors) => {
        ...
        const theSortestLength = Math.min(
          ...rowAncestors.map((ra) => ra.length),
        );
      ...
      const mergeBrotherRows = (
        ...
        const groupRowUid = selectedRows
          .map((d) => d.uid)
          .join(DATARTSEPERATOR);
        const groupRow = {
          uid: groupRowUid,
          colName: groupRowUid,
