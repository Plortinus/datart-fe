...
import { ChartInteractionEvent, DataViewFieldType } from '@/constants';
...
import ChartDataSetDTO, { IChartDataSet } from '@/types/ChartDataSet';
import { BrokerContext, BrokerOption } from '@/types/ChartLifecycleBroker';
import {
  getColumnRenderName,
...
import { precisionCalculation } from '@/utils/number';
import { CloneValueDeep, Omit, isEmptyArray } from '@/utils/object';
        ...
        cell: (props) => {
          ...
          const _getBodyTextAlignStyle = (alignValue) => {
            ...
            if (bodyTextAlign === 'default') {
              const type = mixedSectionConfigRows.find((v) => v.uid === uid)
                ?.type;
              if (type === 'NUMERIC') {
                return 'right';
              }
