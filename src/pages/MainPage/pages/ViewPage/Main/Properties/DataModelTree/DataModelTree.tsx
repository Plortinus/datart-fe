...
import { DataViewFieldType, DateFormat } from '@/constants';
import useI18NPrefix from '@/hooks/useI18NPrefix';
import useStateModal, { StateModalSize } from '@/hooks/useStateModal';
import { APP_CURRENT_VERSION } from '@/migration/constants';
import ChartComputedFieldSettingPanel from '@/pages/ChartWorkbenchPage/components/ChartOperationPanel/components/ChartDataViewPanel/components/ChartComputedFieldSettingPanel';
...
const DataModelTree: FC = memo(() => {
  ...
  const handleNodeTypeChange = (type: string[], name) => {
    ...
    const targetBranch = tableColumns?.find(
      (b) => b?.children?.find((bn) => bn.name === name),
    );
    if (!!targetBranch) {
  ...
  const moveNode = (
    ...
    if (nodeIndex !== undefined && nodeIndex > -1) {
      ...
      const branch = columns?.find(
        (c) => c.children?.find((cc) => cc.name === node.name),
      );
      if (branch) {
        branch.children =
