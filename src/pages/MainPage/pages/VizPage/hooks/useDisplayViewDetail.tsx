...
import { ViewDetailSetting } from '@/components/FormGenerator/Customize/Interaction/types';
import { InteractionFieldMapper } from '@/components/FormGenerator/constants';
...
import { fetchChartDataSet } from '@/utils/fetch';
...
const filterTableColumnsByViewDetailSetting = (
  ...
  return datas?.map((section) => {
    const rows = section?.rows?.filter(
      (r) => enableColumns?.includes(r.colName),
    );
