...
import { migrateChartConfig } from '@/migration';
...
import useDisplayJumpVizDialog from '@/pages/MainPage/pages/VizPage/hooks/useDisplayJumpVizDialog';
import useDisplayViewDetail from '@/pages/MainPage/pages/VizPage/hooks/useDisplayViewDetail';
import { selectShareExecuteTokenMap } from '@/pages/SharePage/slice/selectors';
import { IChart } from '@/types/Chart';
import { ChartConfig } from '@/types/ChartConfig';
import { ChartDetailConfigDTO } from '@/types/ChartConfigDTO';
import { IChartDrillOption } from '@/types/ChartDrillOption';
import { mergeToChartConfig } from '@/utils/ChartDtoHelper';
import {
...
export const DataChartWidgetCore: React.FC<{}> = memo(() => {
  ...
  const buildDrillThroughEventParams = useCallback(
    (clickEventParams, targetEvent: InteractionMouseEvent, ruleId?: string) => {
      ...
      const widgetViewQueryVariables = queryVariables?.filter(
        (qv) => widgetRef?.current?.viewIds?.includes(qv?.viewId || ''),
      );
      return {
