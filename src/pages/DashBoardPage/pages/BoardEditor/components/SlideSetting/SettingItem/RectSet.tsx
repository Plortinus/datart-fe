 ...
 */
import { BW } from '@/components/FormGenerator/Basic/components/BasicWrapper';
import useI18NPrefix from '@/hooks/useI18NPrefix';
import { WidgetActionContext } from '@/pages/DashBoardPage/components/ActionProvider/WidgetActionProvider';
...
export const RectSet: FC<{ wid: string; rect: RectConfig }> = memo(
  ({ wid, rect }) => {
    ...
    const changeW = useCallback(
      (val) => changeRect('width', val),
      [changeRect],
    );
    const changeH = useCallback(
      (val) => changeRect('height', val),
      [changeRect],
    );
    return (
