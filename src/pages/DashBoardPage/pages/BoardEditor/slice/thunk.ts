import migrateWidgetChartConfig from '@/migration/BoardConfig/migrateWidgetChartConfig';
import migrateWidgetConfig from '@/migration/BoardConfig/migrateWidgetConfig';
import { migrateWidgets } from '@/migration/BoardConfig/migrateWidgets';
import { ChartDataRequestBuilder } from '@/models/ChartDataRequestBuilder';
import {
  boardDrillManager,
  EDIT_PREFIX,
} from '@/pages/DashBoardPage/components/BoardDrillManager/BoardDrillManager';
import widgetManager from '@/pages/DashBoardPage/components/WidgetManager';
import { getControlOptionQueryParams } from '@/pages/DashBoardPage/components/Widgets/ControllerWidget/config';
import { ORIGINAL_TYPE_MAP } from '@/pages/DashBoardPage/constants';
import { boardActions } from '@/pages/DashBoardPage/pages/Board/slice';
import {
  BoardState,
...
import { getChartWidgetRequestParams } from '@/pages/DashBoardPage/utils';
import {
  getChartDataView,
  getDashBoardByResBoard,
  getDataChartsByServer,
  getInitBoardInfo,
} from '@/pages/DashBoardPage/utils/board';
import {
  convertWrapChartWidget,
  createToSaveWidgetGroup,
  createWidgetInfo,
  getWidgetMap,
} from '@/pages/DashBoardPage/utils/widget';
...
import { filterSqlOperatorName } from '@/utils/internalChartHelper';
...
>(
  ...
  async (dashboardId, { getState, dispatch }) => {
    ...
    const widgetInfos = Object.keys(widgetMap).map((id) =>
      createWidgetInfo(id),
    );
