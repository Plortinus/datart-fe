...
import {
  ...
  JumpToDashboardRule,
  JumpToUrlRule,
} from '@/components/FormGenerator/Customize/Interaction/types';
import {
  InteractionFieldRelation,
  InteractionRelationType,
} from '@/components/FormGenerator/constants';
import {
...
import { VariableTypes } from '@/pages/MainPage/pages/VariablePage/constants';
...
export const filterSqlOperatorName = (requestParams, widgetData) => {
  const sqlOperatorNameList = requestParams.aggregators.map(
    (aggConfig) => aggConfig.sqlOperator?.toLocaleLowerCase(),
  );
