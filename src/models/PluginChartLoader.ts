...
import * as datartChartHelper from '@/utils/chartHelper';
import { fetchPluginChart } from '@/utils/fetch';
  ...
  async loadPlugins(paths: string[]) {
    const loadPluginTasks = paths.map(async (path) => {
      try {
         ...
         * Git Issue: https://github.com/facebook/create-react-@/issues/5563
