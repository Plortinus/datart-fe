...
import { ChartStyleConfigDTO } from '@/types/ChartConfigDTO';
import {
  ChartDataRequest,
  ChartDataRequestFilter,
  PendingChartDataRequestFilter,
} from '@/types/ChartDataRequest';
import { ChartDatasetPageInfo } from '@/types/ChartDataSet';
    ...
    const aggColumns = this.chartDataConfigs.reduce<ChartDataSectionField[]>(
      (acc, cur) => {
        ...
        if (
          ...
          cur.rows?.findIndex((v) => v.type === DataViewFieldType.NUMERIC) !==
            -1
        ) {
          return acc.concat(
